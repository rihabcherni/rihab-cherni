...
const Experience = ({ t, isDark, visibleSections, isRTL }) => {
  ...
  const lineVariants = {
    hidden: { scaleY: 0 },
    visible: { scaleY: 1, transition: { duration: 1.5, ease: "easeInOut", delay: 0.3 } }
  };
  const horizontalLineVariants = {
    hidden: { scaleX: 0 },
    visible: { scaleX: 1, transition: { duration: 1.2, ease: "easeInOut", delay: 0.3 } }
  };
                  ...
                  <motion.div key={index} variants={itemVariants} className="relative flex-1 min-w-0 group px-1" style={{ marginLeft: isRTL ? isEven ? '-130px' : '0px' : isEven ? '0px' : '-130px', marginRight: isRTL ? isEven ? '0px' : '-130px' : isEven ? '-130px' : '0px', maxWidth: `${100 / t.experience.listeExperiences.length}%` }} whileHover={{ scale: 1.02, transition: { duration: 0.2 } }}>
                    ...
                    <motion.div
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="mb-3">
                          <motion.h3 className="text-[14px] font-bold mb-2 line-clamp-2 leading-tight" whileHover={{ x: 2 }} transition={{ duration: 0.2 }}>
                            {exp.title}
                          </motion.h3>
                          <motion.p className="text-blue-500 font-semibold text-[13px] mb-1" whileHover={{ scale: 1.02 }} transition={{ duration: 0.2 }}>
                            {exp.company}
                          </motion.p>
                        ...
                        <div className="mb-3">
                          ...
                          {exp.description.length > 100 && (
                            <button aria-label="View btn" onClick={() => toggleExpanded(index)} className="text-blue-500 hover:text-blue-600 text-xs mt-1 flex items-center gap-1 transition-colors">
                              {expandedCards[index] ? (<>{t.experience.moins} <ChevronUp className="w-3 h-3" /></>) : (<>{t.experience.plus}<ChevronDown className="w-3 h-3" /></>)}
                            </button>
                          )}
                        </div>
                        <div>
